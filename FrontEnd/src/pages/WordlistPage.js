...
function WordlistPage({ client, user }) {
  ...
  function onSubmit() {
    ...
    for (let i = 1; i <= 10; i++) {
      ...
      newWord.setWord(wordlistState[`word${i}`].replace(/\s/g, '').toLowerCase());
    ...
    <div className="container">
      <div className="box">
        <div className="wordlist-header">
          ...
          {success ? (
            <>
              <h1>Wordlist Created!</h1>
              {/* div to display buttons vertically spaced */}
              <Stack spacing={3}>
                <Button variant="contained" onClick={() => setSuccess(false)}>
                  {' '}
                  Create Another Wordlist{' '}
                </Button>

                <Button variant="contained" onClick={() => navigate('/wordlist-overview')}>
                  {' '}
                  View Your Wordlists{' '}
                </Button>
